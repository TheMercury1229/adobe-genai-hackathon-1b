...
class IntegratedSemanticSearch:
    ...
    def process_all_pdfs_and_extract(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not results or 'pdf_sections' not in results:
                self.logger.error("No PDF sections extracted from existing pipeline")
                return {}
            
            self.logger.info(f"Successfully processed {len(results['pdf_sections'])} PDFs")
            return results
    ...
    def prepare_sections_for_search(self, pdf_extraction_results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"Preparing {len(pdf_sections)} PDFs for semantic search...")
        ...
        for filename, sections_list in pdf_sections.items():
            ...
            for section in sections_list:
                ...
                if section_data['section_text'] and len(section_data['section_text']) > 50:
                    ...
                    self.logger.warning(f"Skipping section with insufficient content: {section_data['section_title']}")
        
        self.logger.info(f"Prepared {len(all_sections)} sections for semantic search")
        return all_sections
    ...
    def find_most_relevant_sections(
        ...
        try:
            ...
            self.logger.info(f"Computing embeddings for {len(search_texts)} sections...")
    ...
    def _keyword_fallback_search(self, sections_data: List[Dict[str, Any]], query: str, top_k: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(f"Keyword fallback found {len(top_results)} results")
            return top_results
    ...
    def format_output_like_sample(
        ...
        job_to_be_done = pdf_results.get('job_to_be_done', 'Plan a trip of 4 days for a group of 10 college friends.')
    ...
    def complete_pipeline_search(
        ...
        output_filename: str = "semantic_search_results.json"
    ) -> Dict[str, Any]:
        """
        ...
        persona = pdf_results.get('persona', 'Travel Planner')
        job_to_be_done = pdf_results.get('job_to_be_done', 'Plan a trip of 4 days for a group of 10 college friends.')
        collection_folder = pdf_results.get('collection_folder', self.collection_folder)
        ...
        self.logger.info(f"Performing semantic search for top {top_k} results...")
        ...
        formatted_results = self.format_output_like_sample(pdf_results, search_results)
        ...
        self.print_formatted_summary(formatted_results)
    ...
    def save_formatted_results(self, results: Dict[str, Any], output_filename: str):
        ...
        try:
            ...
            self.logger.info(f"Collection folder created/verified: {collection_path}")
            ...
            if os.path.exists(output_path):
                ...
                self.logger.info(f"✅ File saved successfully: {output_path} ({file_size} bytes)")
            ...
            try:
                ...
                with open(fallback_path, 'w', encoding='utf-8') as f:
                    json.dump(results, f, indent=4, ensure_ascii=False)
                print(f"💡 Fallback: Saved to current directory: {fallback_path}")
    ...
    def save_text_summary(self, results: Dict[str, Any], summary_path: str):
        ...
        try:
            with open(summary_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"  • Collection folder: {metadata.get('collection_folder', 'N/A')}\n")
                f.write(f"  • Persona: {metadata.get('persona', 'N/A')}\n")
                f.write(f"  • Job: {metadata.get('job_to_be_done', 'N/A')}\n")
                f.write(f"  • Documents processed: {len(metadata.get('input_documents', []))}\n")
                f.write(f"  • Processing time: {metadata.get('processing_timestamp', 'N/A')}\n\n")
                ...
                for section in results.get('extracted_sections', []):
                    f.write(f"{section['importance_rank']}. {section['section_title']}\n")
                    f.write(f"   Document: {section['document']}\n")
                    f.write(f"   Page: {section['page_number']}\n")
                    f.write(f"   Similarity Score: {section.get('similarity_score', 0.0):.4f}\n\n")
                ...
                for i, analysis in enumerate(results.get('subsection_analysis', []), 1):
                    ...
                    f.write(f"   Similarity Score: {analysis.get('similarity_score', 0.0):.4f}\n")
                    f.write(f"   Content Preview: {analysis['refined_text'][:200]}...\n")
                    f.write("-"*60 + "\n\n")
    ...
    def print_formatted_summary(self, results: Dict[str, Any]):
        ...
        for section in extracted_sections:
            ...
            print(f"   📁 {section['document']} (Page {section['page_number']}) - Score: {section.get('similarity_score', 0.0):.4f}")
        
        print(f"\n📊 Results saved to {self.collection_folder}/ folder")
        print(f"📋 Check the JSON file for complete structured data")
        print(f"📝 Check the summary file for readable overview")
        print("\n" + "="*80)


def process_collection_with_semantic_search(collection_folder: str, top_k: int = 5) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        results = search_system.complete_pipeline_search(
            top_k=top_k,
            save_results=True,
            output_filename="semantic_search_results.json"
        )
...
def main():
    ...
    try:
        results = process_collection_with_semantic_search(default_collection)
        
        if results:
            print(f"\n📁 Results saved to {default_collection}/semantic_search_results.json")
            print(f"📋 Summary saved to {default_collection}/semantic_search_results_summary.txt")
        else:
            print("\n💡 Try the following:")
            print("  • Check your internet connection")
            print("  • Run: pip install sentence-transformers scikit-learn")
            print(f"  • Ensure {default_collection} folder exists with PDF files")
